...
import argparse
import visualsearch.visual_searcher as vs
import sys
from os import path
...
def setup_and_run(dataset_name, config_name, image_name, image_range, human_subject, number_of_processes, save_probability_maps):
     ...
     output_path  = path.join(constants.RESULTS_PATH, path.join(dataset_name + '_dataset', config_name))
     ...
     visual_searcher = vs.VisualSearcher(config, dataset_info, trials_properties, output_path, human_scanpaths,constants.SIGMA)
     visual_searcher.run()

""" Main method, added to be polymorphic with respect to the other models """
def main(dataset_name, human_subject=None):
     setup_and_run(dataset_name, config_name='greedy_hsp', image_name=None, image_range=None, human_subject=human_subject, number_of_processes='all', save_probability_maps=True)
